...
function bereinigeZutaten(zut: any[], schlank = false): { zutaten: any[]; warnungen: string[] } {
  const w: string[] = [];
  if (!Array.isArray(zut)) return { zutaten: [], warnungen: w };
  let z = zut.filter((x) => {
    /* 23.09.2026 (Ralph jaja Foto-Tempo B): im Schlank-Modus liefert Riki keine Note - die Maschine
       bewertet ohnehin aus dem Stamm. Dann zaehlt nur: hat die Zeile einen Namen. */
    if (schlank) return !!x?.name;
...
Deno.serve(async (req) => {
  ...
  try{
    ...
    inhalt.push({type:"text",text:"Lies aus diesen Etikettfotos die Nährwerttabelle (Spalte 'je 100 g'!), die Zutatenliste UND — falls vorhanden — die Wirkstoff-/Vitamin-Tabelle mit Mengen aus."+(body.ean?(body.ean_pruefen===true?" Das Produkt hat einen Barcode; lies seine Nummer selbst vom Foto ab (Feld ean_auf_foto).":` Der Barcode lautet ${body.ean}.`):" Dieses Produkt hat keinen Barcode.")+" Zutaten nur einmal aus der deutschen Fassung. Strukturiere jede Zutatenzeile nach Work #78 in original_text, base_ingredient, processing_modifiers, attributes, parenthetical_role, parenthetical_items und extraction_status. Klammerinhalt erst nach seiner Rolle beurteilen; composition/explanation erzeugt keine weiteren Produktzutaten. Unklar => unresolved. Einheiten nie umdeuten."+(body.schlank===true?" SCHLANK-MODUS (Vorrang vor dem Regelwerk): Lass bei jeder Zutat die Felder rating, kritisch und begruendung KOMPLETT weg - die Bewertung macht Root Index selbst. Gib trotzdem JEDE echte Zutat aus, auch wenn du sie nicht bewerten kannst. warnungen: hoechstens 2 kurze Saetze, sonst []. Keine Erklaerungen, nur das JSON.":"")+(body.nur_zeilen===true?" NUR-ZEILEN-MODUS (Vorrang vor Regelwerk und Work #78): Jede Zutat NUR als {\"name\": deutscher Kurzname, \"original_text\": Wortlaut wie auf dem Etikett inkl. Klammer und Prozent}. KEINE weiteren Felder je Zutat (kein base_ingredient, processing_modifiers, attributes, parenthetical_*, extraction_status, anteil_prozent, rating, kritisch, begruendung). Root Index zerlegt die Zeilen selbst. wirkstoffe und mikronaehrstoffe_100g nur, wenn wirklich eine Tabelle dafuer da ist, sonst [].":"")});
    ...
    const clean=bereinigeZutaten(vorschlag.zutaten??[], body.schlank===true||body.nur_zeilen===true);vorschlag.zutaten=clean.zutaten;vorschlag.wirkstoffe=bereinigeWirkstoffe(vorschlag.wirkstoffe??[]);try{vorschlag.mikronaehrstoffe_100g=bereinigeMikro100(vorschlag.mikronaehrstoffe_100g??[]);}catch{vorschlag.mikronaehrstoffe_100g=[];}
